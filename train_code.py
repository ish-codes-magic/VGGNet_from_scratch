...
from my_utils import load_pretrained_state_dict, load_resume_state_dict, make_directory, save_checkpoint, Summary, AverageMeter, ProgressMeter, accuracy
load_dotenv()
...
def build_model(num_classes,model_arch,model_ema_decay,device) -> [nn.Module, nn.Module]:
    vgg_model = model.__dict__[model_arch](num_classes=num_classes)
...
def train(train_loader, vgg_model, ema_vgg_model, criterion, optimizer, device, scaler, epoch, writer, train_print_frequency):
    ...
    return losses.avg, acc1.avg.item(), acc5.avg.item()
...
def main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        loss_train, train_acc1, train_acc5 = train(train_loader, vgg_model, ema_vgg_model, criterion, optimizer, device, scaler, epoch, writer_train, train_print_frequency)
